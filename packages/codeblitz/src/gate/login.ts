 ...
 * 跳转: `{LOGIN_REDIRECT}{encodeURIComponent(当前URL)}&needToken=true`
 *   例: https://beta.cloudlab.top/login?redirect=http%3A%2F%2Fhost%3A7788%2F&needToken=true
 *
...
/** 未登录则跳转登录页 (带 redirect=自身URL + needToken=true). 应在渲染前调用. */
export function enforceLogin(): void {
  ...
  window.location.replace(`${base}${encodeURIComponent(here)}&needToken=true`);
}
