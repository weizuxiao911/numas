...
// shiki (含 ~3MB oniguruma wasm) / katex 懒加载 + 管线懒建: 首次渲染 markdown 时才拉取, 不拖启动
let pipelinePromise: Promise<import('marked').Marked> | null = null;
function getPipeline(): Promise<import('marked').Marked> {
  pipelinePromise ??= (async () => {
    const [{ codeToHtml }, { default: markedKatex }] = await Promise.all([
      import('shiki'),
