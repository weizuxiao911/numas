  ...
  const buildPrompt = useCallback((kind: 'animation' | 'code', a: Annotation, productAbsPath: string, annoAbsPath: string): string => {
    ...
    const task = kind === 'animation'
      ...
      : [
          ...
          '要求:',
          '1. **必须先读取源文件 (PDF) 对应页的完整内容**, 根据文档内容与圈选内容判定该知识点涉及的编程语言 (文档/圈选内容明确提到 Python/Java/C++/Go/JS 等 → 用该语言; 均未明确时才用 Python); 不得仅凭关键词猜测',
          `2. 代码实现必须紧扣文档内容 (该页讲解的算法/概念/公式), 结合圈选内容生成可运行示例 (关键字初筛参考: ${detectLang(a.text).lang}, 最终以文档内容为准)`,
          '3. 产物文件扩展名必须与最终判定的语言一致; 若与给定产物路径的扩展名不符, 按正确扩展名写入并在回填 anno 时使用实际路径',
          '4. 代码自包含, 不依赖外部服务 (需要三方库时在文件头注释安装命令)',
          '5. 关键逻辑有中文注释; 运行后打印清晰结果',
          '6. 文件首行必须是运行命令注释, 格式 `<语言注释符> run: <完整运行命令>` (例: `# run: python3 /abs/xxx.py`; `// run: javac /abs/Main.java && java -cp /abs Main`)',
          '7. 只输出代码文件内容, 不要解释',
        ].join('\n');
    return [
      head,
